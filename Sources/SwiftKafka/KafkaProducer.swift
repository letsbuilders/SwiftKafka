...
public class KafkaProducer: KafkaClient {
    ...
    public func send(producerRecord: KafkaProducerRecord, messageCallback: ((Result<KafkaConsumerRecord, KafkaError>) -> Void)? = nil) {
        ...
        let responseCode = producerRecord.value.withUnsafeBytes { rawBufferPointer -> Int32 in
            let rawPtr = rawBufferPointer.baseAddress!
            return rd_kafka_produce(topicPointer,
                                    producerRecord.partition,
                                    RD_KAFKA_MSG_F_COPY,
                                    UnsafeMutableRawPointer(mutating: rawPtr),
